...
export function EvidenceGrid({ view }: { view: OrchestrationReadModel }) {
  ...
  return <div className="orch-evidence-grid">
    ...
    <section className="orch-card"><h3>Verification</h3>{view.verifications.length ? <ul>{view.verifications.slice(-8).map((record) => <li key={record.id}><span className={`orch-state state-${record.status}`}>{record.status}</span> {record.scope}: {record.outputSummary}</li>)}</ul> : <p>No verification records yet.</p>}</section>
  </div>;
}
